...
--------------------------  Vehicle inspection  --------------------------------
--------------------------------------------------------------------------------
Vehicles arrive at technical inspection. The last check relates to engine
operation. If an engine is defective, the vehicle is sent to the repair 
department. After the repair, the vehicle returns to inspection. After a 
successful inspection, the vehicle goes to the parking lot. At the inspection 
point, where engine operation control is performed, vehicles arrive every 
10 ± 5 min. The vehicle is inspected by two workers. Vehicle inspection time is 
20 ± 5 min. Of all vehicles inspected, 70% of them are in order  and go to 
parking. The other 30% of vehicles go to the repair department, where one worker
works on the repair. The time required to repair the vehicle is 45 ± 15 min.

It is necessary to collect statistics of the vehicle queues at the place of 
inspection and at the place of repair. Also, it is necessary to create 
histograms of:
...
from pyes.base import clock, elapsed_time
...
def init():
  ...
  yield arrival(vehicle()),rn1.uniform(50,150)
  # Schedule terminate simulation event
  yield terminate(),8*60*10
...
def inspection():
  ...
  if insp and que1:
    ...
    yield departure1(v),rn2.uniform(150,250)
...
def repair():
  ...
  if mike and que2:
    ...
    yield departure2(v),rn4.uniform(300,600)
